  ...
  if (contentObject != 0)
    {
      ...
      std::cout<<"IPK-CHIT "<<Simulator::Now ()<<" "<<Names::FindName (pitEntry->GetFibEntry ()->GetFib()->GetObject<Node>())<<" "<<interest->GetName()<<" -1 "<<interest->GetNonce()<<" "<<hopCountTag.Get()<<std::endl;
  ...
  if (similarInterest && ShouldSuppressIncomingInterest (inFace, interest, pitEntry))
    {
      ...
      pitEntry->AddIncoming (inFace, hops);
  ...
  BOOST_FOREACH (const pit::IncomingFace &incoming, pitEntry->GetIncoming ())
    {
	  if (count > 0) {
		  ...
		  Ptr<Packet> tempPacketNew = Wire::FromData(data);
		  		  
		  FwHopCountTag hopCountTag;
	      if (tempPacketNew->PeekPacketTag (hopCountTag))
          {
			  hopCountTag.Set(incoming.m_hopcount);
			  tempPacketNew->ReplacePacketTag (hopCountTag);			  
	      }
	      
	      Ptr<const Data> dataPacket = Wire::ToData(tempPacketNew);	
	      bool ok = incoming.m_face->SendData (dataPacket);

          DidSendOutData (inFace, incoming.m_face, dataPacket, pitEntry);
